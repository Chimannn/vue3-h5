<template>
    <div class="box">
        ...
        class="scroll-box">
            ...
            <slot></slot>
            <div v-if="!props.isScroll" class="end-text">{{ props.endText }}</div>
        </div>
    </div>
</template>
...
const scrollEvent = (e: any) => {
    ...
    if(data.scrollTop + e.srcElement.offsetHeight > e.srcElement.scrollHeight - props.distance){
        emits("getList")
    }
}
...
const touchstart = (e: any) => {
    let y = e.targetTouches[0].pageY;//记录初始位置
    data.startY = y;
};
